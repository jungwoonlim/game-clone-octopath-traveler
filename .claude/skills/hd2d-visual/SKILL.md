...
- **fov 25~35** — 좁을수록 원근이 눌려 미니어처감이 강해진다. 기본값 75는 너무 넓어 그냥 3D로 보인다.
  25 아래로 내려가면 시야가 답답해져 필드 탐험이 불편해지므로 30 근처에서 시작한다.
- **부감 -45°** — 옥토패스의 상징적인 각도. 플레이어를 따라갈 때도 **각도는 고정**하고 위치만 옮긴다.
  각도가 흔들리면 디오라마 느낌이 즉시 깨진다.
- 원근 투영(`projection=0`)을 쓴다. 직교로 하면 원근감이 완전히 사라져 오히려 밋밋해진다.
...
- **밤에 `glow_intensity`를 올린다** (0.85 → 1.15). 블룸이 등불을 번지게 해 분위기를 만든다
