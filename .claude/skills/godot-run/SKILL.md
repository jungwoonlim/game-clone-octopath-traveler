...
**`wait:<프레임>`으로 연출을 기다리지 마라.** 타이핑 같은 연출은 벽시계(초당 30자)로 도는데
`wait:`는 프레임을 세고 창 모드 fps는 72~120으로 흔들린다. 실제로
`press;wait:150;press;wait:150;press;wait:40`이 **패스 액션 목록이 뜨지 않은 화면**을 `✓ PASS`로
저장했고, 그걸 제품 버그로 의심해 한참을 뒤진 끝에 하네스 문제로 밝혀졌다.
연출을 기다릴 때는 `sleep:2`처럼 초로 준다. 대사 페이지를 확실히 넘기려면
**페이지당 `press`를 2회 연속**으로 주는 방법도 있다(타이핑 완성 1회 + 넘김 1회).
...
**`pose` 직후의 `press`는 예전에 항상 삼켜졌다.** 순간이동으로 옮긴 `Area3D`의 겹침은 물리 스텝이
한 번 돌아야 갱신되는데 캡처 도구가 `process_frame` 하나만 기다렸기 때문이다. 그래서 상호작용
후보가 빈 채로 `interact`가 날아가 "대화가 시작되지 않은 화면"이 정상 캡처로 저장됐다.
지금은 `pose` 뒤에 물리 프레임 2회를 기다리므로 `wait:10` 같은 보정을 넣지 않아도 된다.
**그래도 `press`가 안 먹으면 타임라인이 아니라 대상 탐지 로직을 의심하라.**

**매달리면 3분에서 끊는다** (`SHOT_TIMEOUT`으로 조정). 진행 중에는 Godot 출력을 그대로 흘려 보여 준다.
예전에는 출력을 변수에 담아 끝날 때까지 아무것도 보이지 않았고, 살아 있는지조차 알 수 없는 채로
35분을 기다린 적이 있다.

**그리기가 멈추면 자동으로 다시 찍는다** (`SHOT_RETRIES`, 기본 3회). macOS는 창이 가려지면
그리기를 멈추는데, 그러면 `frame_post_draw`가 영영 오지 않아 캡처가 매달린다.
**게임 루프는 120fps로 멀쩡히 돌기 때문에 로그만 봐서는 살아 있는지 알 수 없다** — 이 함정에
검증 세션 하나가 통째로 묶인 적이 있다. 지금은 캡처 창을 항상 위로 띄우고(측정: 실패 10회 중 4회
→ 20회 중 0회), 그래도 멈추면 재시도한다. `✗ FAIL — shot (그리기가 3회 연속으로 멈췄다)`가
뜨면 코드 문제가 아니라 창이 계속 가려져 있는 것이다.

**찍은 화면의 작은 부분(글자·도트 경계)을 판단할 때는 `tools/crop_zoom.gd`로 확대한다.**
640×360 원본을 그대로 보면 폰트 안티에일리어싱 같은 것은 구분이 불가능하고, 부드러운 보간으로
키우면 AA가 있든 없든 똑같이 뿌예져 비교 자체가 성립하지 않는다. 이 도구는 최근접으로 확대한다.
